...
for(tar_ball_path in tar_ball_paths) {
  ...
  if (dir.exists(str_c(params$path, "derived_data/"))) {
    ...
    mutate(target_task = split_name[length(split_name) - 2],
      ...
      rename(epoch = "X")
  i <- i + 1
  system("rm -r derived_data/chtc_092502/derived_data/")
  }
...
for(tar_ball_path in tar_ball_paths) {
  ...
  if (dir.exists(str_c(params$path, "derived_data/"))) {
    ...
    train_pred <- read.csv(str_c(params$path, "derived_data/", 
                                       list.files(str_c(params$path, "derived_data", ""),
                                                  pattern = "train_pred.csv")))
    val_pred <- read.csv(str_c(params$path, "derived_data/", 
                                       list.files(str_c(params$path, "derived_data", ""),
                                                  pattern = "val_pred.csv")))
    
    test_pred <- read.csv(str_c(params$path, "derived_data/", 
                                       list.files(str_c(params$path, "derived_data", ""),
