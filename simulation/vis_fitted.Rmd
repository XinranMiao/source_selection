...
base_dir <- "test/model_lm/derived_data/high_bw/"
```
...
for (t in 1:100) {
  ...
  # coefficients
  train_only_mod <- lm(Y ~ x, train_data[[t]] %>% filter(name == "y"))
  bandit_coef <- read.csv(str_c(base_dir, "current", t - 1, "/coef.csv")) %>%
    dplyr::select( - X) %>%
   unlist
  coef_list[[t]] <- data.frame(intercept = bandit_coef[1], slope = bandit_coef[2], type = "bandit_selection") %>%
   rbind(data.frame(intercept = train_only_mod$coefficients[1],
                    slope = train_only_mod$coefficients[2],
                    type = "train data only")) %>%
   mutate(t = t)
}
```
...
coef_list %>%
  ...
  mutate(t = as.factor(t)) %>%
  ggplot() +
  geom_abline(aes(slope = slope, intercept = intercept, color = type), size = 2) +
  facet_wrap(as.factor(t) ~ ., nrow = 1) +
  theme(legend.position = "bottom")
```

```{r,fig.width=6,fig.height=6}
loss <- read.csv(str_c(base_dir, "losses.csv")) %>%
    rename(iteration = X) %>%
          rename(value = losses,
                 task = bandit_selects) %>%
