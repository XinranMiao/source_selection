...
source_split = function(df, # data.frame withe the first column indicating predictable variable
  ...
  if(plotting){
    ...
    ggsave(paste0(output_dir,'source_split','.png'),height = 6,width = 8)
    } # data_type == 'California'
  }# if plotting
...
# input ---
# output ---
pred_func = function(source,  val, mod = 'loglm'){
  if(mod == 'lm'){
...
plot_ensemble = function(dat,
  ...
  names(dat_plot) = c('y',
                      sapply(1:n_cluster,function(x){paste0('Cluster',x)}) %>% 
                        unlist)
  if(! is.null(additional_dir)){
    jpeg(paste0(output_dir,'ensemble',additional_dir,'.png'))
  }
  superheat(dplyr::select(dat_plot, -y), 
            # scale the variables/columns
            # scale = TRUE,
