...
output_base_dir = 'trans_'
output_dir = output_base_dir + 'target' + str(which_target)+ "_K"+str(K)+'_seed'+str(seed)+'_'+split_criteria + '_' + feature+ "_P"+str(proportion)+'/'
...
df_clusters = split_source_transition(K=K, # positive integer: number of clusters
                 ...
                 which_target = which_target, # an integer or list of row indices. split is on the rest of data
...
model.fc = Linear(in_features = 512, out_features = 2,bias = True)
model.load_state_dict(torch.load('model_1211_0.7acc.pt', map_location=device)) # added
...
for t in range(T):
    ...
    Training_samples[t] = train_sample_t
    train_sample = train_sample_t #+train_sample
    ...
    l,a = train_model(train_subset_train_loader,model,criterion,optimizer,
    n_epochs= n_epochs,batch_size = batch_size, device = device)
   
    # evaluate
    with torch.no_grad():
        val_acc,val_pred,val_prob =  pred_acc(train_subset_val_loader,model,batch_size=batch_size,device = device)
        test_acc,test_pred,test_prob = pred_acc(test_subset_loader,model,batch_size=batch_size,device = device)
    ...
    if t % 20 == 0:
        Training_samples.to_csv(output_dir+'Training_samples_sofar.csv')
        pd.DataFrame({
            'Y':Y,
            ...
            'Validation_Accuracy':val_Accuracy
                    }).to_csv(output_dir+'/bandit_T_sample_2.csv')
...
Training_samples.to_csv(output_dir+'Training_samples.csv')
torch.save(model.state_dict(), output_dir+'bandit_model.pt')
